...
def collect():
    ...
    go_license=next((candidate for candidate in (go_root/'LICENSE',go_root.parent/'LICENSE',go_root.parent.parent/'LICENSE') if candidate.is_file()),None)
    if go_license is None: raise RuntimeError(f'No Go toolchain license near {go_root}')
