...
- The gateway binds a loopback IP and requires a local Bearer token by default.
  An explicit `allow_external` configuration can bind a non-loopback IP for LAN
  access and emits a warning; this mode is plain HTTP and must be protected by
  a trusted network, firewall, and preferably a TLS-terminating reverse proxy.
