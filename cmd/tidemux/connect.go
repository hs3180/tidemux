...
func launch(args []string, stdout, stderr *os.File) error {
	...
	if err != nil {
		...
		if ttyErr != nil {
			return errors.New("local gateway credential unavailable; run the client command in Terminal to unlock Keychain, or run `tidemux gateway configure --rotate-key` if the credential is missing")
		}
		...
		token, err = (gateway.MacOSKeychain{}).Lookup(ctx, c.AccessTokenKeychain)
		if err != nil {
			return errors.New("local gateway credential unavailable after Keychain check; run `tidemux gateway configure --rotate-key` for this profile")
		}
	}
