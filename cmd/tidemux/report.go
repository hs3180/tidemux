...
func report(args []string, stdout, stderr *os.File) error {
	...
	command := args[0]
	if command != "generate" && command != "list" && command != "export" && command != "deliver" && command != "retry" {
		return errors.New(usage)
	}
	...
	output := flags.String("output", "", "HTML export path")
	...
	if command == "export" {
		if strings.TrimSpace(*output) == "" || *days < 1 || *days > 3660 {
			return errors.New("report export requires --output and --days 1..3660")
		}
		loc, err := time.LoadLocation(*timezone)
		if err != nil {
			return errors.New("invalid report timezone")
		...
		result, err := exportHTMLReport(context.Background(), l, *output, *timezone, through, *days, ledger.ReportBudget{Currency: *budgetCurrency, DailyLimit: *dailyBudget}, *openReport)
		if err != nil {
			return err
		}
