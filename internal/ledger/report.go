...
func (l *Ledger) GenerateDailyReport(ctx context.Context, day time.Time, timezone string, budget BudgetPolicy) (DailyReport, error) {
 ...
 COALESCE(SUM(CASE WHEN estimated_cost IS NULL THEN 1 ELSE 0 END),0), COALESCE(SUM(estimated_cost),0), COALESCE(MAX(currency),'') FROM request_audit WHERE timestamp_ms>=? AND timestamp_ms<?`, local.UnixMilli(), next.UnixMilli()).Scan(&r.RequestCount, &r.FailureCount, &r.InputTokens, &r.OutputTokens, &r.UnknownCostRequests, &estimated, &currency)
	...
	_ = l.db.QueryRowContext(ctx, `SELECT COALESCE(SUM(CASE WHEN c.estimated_input<>COALESCE(a.input_tokens,-1) OR c.estimated_output<>COALESCE(a.output_tokens,-1) THEN 1 ELSE 0 END),0) FROM token_comparisons c JOIN request_audit a ON a.id=c.request_id WHERE a.timestamp_ms>=? AND a.timestamp_ms<?`, local.UnixMilli(), next.UnixMilli()).Scan(&r.TokenizerMismatches)
	_ = l.db.QueryRowContext(ctx, `SELECT COALESCE(COUNT(*),0) FROM supplier_statement_lines s WHERE s.period_start_ms>=? AND s.period_start_ms<? AND (s.request_id IS NULL OR NOT EXISTS(SELECT 1 FROM request_audit a WHERE a.id=s.request_id))`, local.UnixMilli(), next.UnixMilli()).Scan(&r.UnmatchedStatements)
	if r.Currency != "" {
		var first, last float64
		if l.db.QueryRowContext(ctx, `SELECT total FROM balance_snapshots WHERE currency=? AND observed_at_ms>=? AND observed_at_ms<? ORDER BY observed_at_ms LIMIT 1`, r.Currency, local.UnixMilli(), next.UnixMilli()).Scan(&first) == nil && l.db.QueryRowContext(ctx, `SELECT total FROM balance_snapshots WHERE currency=? AND observed_at_ms>=? AND observed_at_ms<? ORDER BY observed_at_ms DESC LIMIT 1`, r.Currency, local.UnixMilli(), next.UnixMilli()).Scan(&last) == nil {
			d := last - first
			r.BalanceNetChange = &d
		}
	}
	if budget != (BudgetPolicy{}) && budget.Currency == r.Currency {
		var spent float64
		_ = l.db.QueryRowContext(ctx, `SELECT COALESCE(SUM(charged_amount),0) FROM budget_reservations WHERE currency=? AND reserved_at_ms>=? AND reserved_at_ms<?`, budget.Currency, local.UnixMilli(), next.UnixMilli()).Scan(&spent)
	...
	result, err := l.db.ExecContext(ctx, `INSERT INTO daily_reports(day,timezone,report_json,created_at_ms) VALUES (?,?,?,?) ON CONFLICT(day,timezone) DO UPDATE SET report_json=excluded.report_json,created_at_ms=excluded.created_at_ms`, r.Day, r.Timezone, string(b), time.Now().UnixMilli())
	if err != nil {
		return r, err
	}
	if id, e := result.LastInsertId(); e == nil {
		...
		_ = l.db.QueryRowContext(ctx, `SELECT id FROM daily_reports WHERE day=? AND timezone=?`, r.Day, r.Timezone).Scan(&r.ID)
	}
	return r, nil
}
