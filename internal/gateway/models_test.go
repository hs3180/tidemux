	...
	for _, protocol := range []string{"openai", "anthropic"} {
		...
		for _, path := range []string{"/v1/models", "/models"} {
			for _, authorized := range []bool{false, true} {
				...
				if json.Unmarshal(w.Body.Bytes(), &result) != nil || len(result.Data) != 1 || result.Data[0].ID != c.Model {
...
func TestModelDetailUsesConfiguredIdentity(t *testing.T) {
	for _, protocol := range []string{"openai", "anthropic"} {
		...
		defer closeDB()
		for _, path := range []string{"/v1/models/", "/models/"} {
			for _, name := range []string{c.Model, "missing"} {
				...
				if w.Code != 200 || json.Unmarshal(w.Body.Bytes(), &body) != nil || body["id"] != c.Model {
